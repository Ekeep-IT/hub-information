...
app.post('/api/repos/orgs', async (req, res) => {
	try {
		...
		const resultOfPromise = await Promise.all(req.body.map(async organisation => {
			const repos = await octokit.request(`GET /orgs/${organisation}/repos`);
			const { data } = repos;
			return data.map((item) => ({ name: item.name, fullName: item.full_name }));
		}));

		const resultFormatted = resultOfPromise.reduce((accumulator, element)=> {return element})
